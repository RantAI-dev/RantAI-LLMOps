...
## INSIDEN: deploy v0.40.36 menimpa DB Transformer Lab, riwayat job HILANG (2026-07-21 12:30 WIB)
**Apa yang terjadi:** saat deploy vLLM-as-stack-service (v0.40.36), aku PUT ulang stack dgn `PullImage:true`. Rilis v0.40.36 IKUT membangun ulang backend (`:latest`), yang ternyata **Transformer Lab versi lebih baru** (skema DB beda total: `teams/quotas/workflows/job_queue`, tanpa `job/experiment` lama). Backend baru membuat `llmlab.sqlite3` KOSONG (skema baru) menimpa DB lama. **API /api/finetune/jobs & /api/evals/jobs -> 0.** Chat history & daftar riwayat di UI hilang.
**AKAR = kesalahanku:** `PullImage:true` ikut meng-upgrade BACKEND (stateful, memegang DB TL). Deploy bedah (hanya tarik frontend) akan mencegahnya. Backend image dipin `:latest` -> rentan auto-upgrade.
**YANG SELAMAT (dikonfirmasi dari disk, tidak ada yg dihapus):** `rantai_merged` 20GB (4 fine-tune); `local_provider/local_provider_runs/orgs/*/` **293 folder run** (log+output+skor SETIAP job); model Ollama 27GB; volume tl_data/ollama_data/ui_data dibuat 16 Juli (BUKAN dibuat ulang).
**YANG HILANG (tidak bisa dipulihkan):** DB indeks yg membuat UI menampilkan daftar job/eval. DB lama tertimpa in-place, **TIDAK ADA backup** (dikonfirmasi user + find: cuma 1 llmlab.sqlite3). Downgrade backend TAK menolong (DB sudah tertimpa). Model & data run tetap bisa dipakai; yg hilang cuma DAFTAR riwayatnya.
**PERBAIKAN (dikerjakan): PIN backend by digest.** `docker-compose.portainer.yml`: `rantai-backend` & `rantai-gateway` image default `:latest` -> `@sha256:d4c323c76cbe7caf4f8c7eecbf9f052ef389bf38bdd104c380563134cd49d4be` (backend yg sedang jalan). Komentar keras di atasnya menjelaskan insiden + cara upgrade sengaja (build+test+BACKUP tl_data dulu). Redeploy ke depan TIDAK akan lagi meng-upgrade TL diam-diam.
**ATURAN DEPLOY BARU (WAJIB dipatuhi):**
1. JANGAN pernah `PullImage:true` buta pada stack yg memuat backend TL, KECUALI backend sudah dipin digest.
...
## 2026-07-22 12:05 WIB — Deploy v0.40.43 (revert host-find) + hapus job gagal
Job gagal 67e6f2e0 dihapus via DELETE /api/finetune/jobs (ok:true) → dropdown 9 fine-tune, a082acda tetap. Deploy v0.40.43 (revert host-find) ke Portainer stack 21 (PUT 200), backend tetap pinned d4c323, pw.txt dipakai sekali lalu dihapus. Verifikasi: frontend 200, 67e6f2e0 tetap hilang, 12 eval job (auth utuh). Keadaan akhir bersih: pesan error ramah + background submit + auto-switch tab live; job gagal baru auto-FAILED jadi tak muncul.

## 2026-07-27 02:38 WIB — UI production polish: seluruh UI jadi English-only + info berlebih → tooltip ⓘ
Persiapan produksi: user minta UI konsisten full English (biar profesional & tak salah-informasi) dan semua teks penjelasan berlebih dipindah ke tooltip hover. Direncanakan lewat plan mode (3 Explore agent: inventaris string, kandidat tooltip, konvensi komponen).
Fondasi: komponen baru `src/components/ui/tooltip.tsx` (Tooltip + InfoTip ikon ⓘ) di atas `@base-ui/react/tooltip`, mengikuti pola shadcn repo (dialog/dropdown-menu), sadar tema light/dark; `tooltip.test.tsx` (2 test); `TooltipProvider` dipasang sekali di `(app)/layout.tsx`; glosarium `docs/i18n-glossary.md`.
Eksekusi: modul serve dikerjakan dulu sebagai contoh gaya (disetujui user), lalu sweep sisanya via 7 agen paralel (evals, hub, finetune, notes+playground, workflows+generations, datasets+settings+registry+llm-ops, app+components+api). ~82 file diubah, ~33 InfoTip ditambah (subtitle header halaman + helper verbose). Locale `id-ID` → `en-US` untuk format tanggal/angka konsisten.
PENTING — logika Indonesia fungsional DIPERTAHANKAN (tak diterjemah): `src/lib/grounding-eval.ts` (system-prompt model, REFUSAL_PATTERNS, refusal string, parseJenjang/parseSource, SD/SMP/SMA) + contoh dataset Indonesia di placeholder grounding-eval.tsx — menerjemahkan ini akan memecah skoring grounding eval & perilaku model tutor.
Test yang mengasert string lama diperbarui (finetune.test.ts: /Baris N/→/Row N/, /duplikat/→/duplicate/). Verifikasi: `tsc --noEmit` bersih, `vitest` 143/143 lolos, `eslint` bersih di semua file yang diubah (1 error pre-existing di use-evals.ts yang TAK disentuh, di luar scope), grep sisa Indonesia = hanya komentar kode + logika grounding fungsional.
Status: belum di-deploy — menunggu review visual user (`npm run dev`) lalu push+release + deploy Portainer (backend tetap pinned).

## 2026-07-27 02:46 WIB — Koreksi pasca-review UI polish (locale + eslint)
Atas permintaan user: (1) locale tanggal/angka DIKEMBALIKAN ke `id-ID` di 4 titik yang sempat diubah ke en-US saat sweep (eval-form.tsx, eval-job-list.tsx, grounding-eval.tsx timestamp, workflow-history.tsx timestamp) — jadi format ID + label WIB dipertahankan. Catatan: en-US pre-existing di dashboard/tl-datetime.ts/datasets/tasks/model-registry TIDAK diubah (bukan bagian sweep). (2) eslint error pre-existing di use-evals.ts:96 (react-hooks/set-state-in-effect) diperbaiki dengan `eslint-disable-next-line` + justifikasi (pola sama seperti gateway-access.tsx) — effect ini sah merekonsiliasi state polling. Verifikasi ulang: eslint 0 error di seluruh src, tsc bersih, vitest 143/143 lolos.

## 2026-07-27 03:01 WIB — Seragamkan SEMUA locale tampilan ke id-ID
Lanjutan permintaan user: semua format waktu/angka disamakan ke `id-ID`, termasuk yang pre-existing en-US. 9 titik diubah: dashboard/page.tsx (fmtNum), lib/tl-datetime.ts (formatter WIB), datasets/lib/utils.ts (3), model-registry/lib/utils.ts (2), tasks/lib/utils.ts (2). Tak ada test yang mengasert output terformat → aman. Verifikasi: 0 sisa "en-US" di src, tsc bersih, eslint 0 error, vitest 143/143. Kini seluruh UI: teks English + format tanggal/angka/WIB ala Indonesia.
