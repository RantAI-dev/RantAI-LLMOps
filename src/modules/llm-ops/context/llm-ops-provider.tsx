...
const ZERO_RESOURCE: TaskResourceUsage = {
  gpu: 0,
  vram: 0,
  cpu: 0,
  memory: 0,
...
function appendRunLog(run: TaskRun, message: string): TaskRun {
  return { ...run, logs: [...run.logs, { time: formatLogTime(), message }] };
}

function runningResourceUsage(task: Task, base: TaskResourceUsage): TaskResourceUsage {
  return {
    ...base,
...
function completeRun(task: Task, run: TaskRun): TaskRun {
...
function startNewRun(task: Task, status: "Running" | "Retrying"): TaskRun {
...
export function LlmOpsProvider({ children }: { children: ReactNode }) {
  ...
  const value: LlmOpsContextValue = {
    tasks,
    experiments,
    getExperimentById,
    taskFilters,
    setTaskFilters,
    resetTaskFilters: () => setTaskFilters(defaultTaskFilters),
    filteredTasks,
    selectedTaskId,
    setSelectedTaskId,
    selectedTask,
    ...
    resetExperimentFilters: () => setExperimentFilters(defaultExperimentFilters),
    filteredExperiments,
    selectedExperimentId,
