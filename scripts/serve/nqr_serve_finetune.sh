...
# Usage: nqr_serve_finetune.sh <jobId> <base_model_hf_id> <ollama_tag>
...
if [ -z "$JOB" ] || [ -z "$BASE" ] || [ -z "$TAG" ]; then
  echo "usage: nqr_serve_finetune.sh <jobId> <base_model> <tag>"; exit 2
fi
...
echo "adapter: $ADIR"
exec bash "$HOME/nqr_export_gguf.sh" "$ADIR" "$BASE" "$TAG"
